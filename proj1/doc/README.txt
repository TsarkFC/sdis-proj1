...
Running the peer:
    ...
    - Using the peer.sh script:
        ...
        - There are some extra commands needed in comparison with the script:
            - Start the RMI - ./startRmi.sh <port>
            - Compile project - ./compile.sh
            - Run peer - ./peer.sh <version> <peer_id> <svc_access_point> <mc_addr> <mc_port> <mdb_addr> <mdb_port> <mdr_addr> <mdr_port>

Running the client:
    - Using testApp.sh available in proj1/src/
        - Usage: ./testApp.sh <peer_ap> <sub_protocol> <opnd_1> <opnd_2>
